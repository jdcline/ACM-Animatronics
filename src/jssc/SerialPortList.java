...
public class SerialPortList {
	...
	static {
		...
		case SerialNativeInterface.OS_MAC_OS_X: {
			// PORTNAMES_REGEXP =
			// Pattern.compile("tty.(serial|usbserial|usbmodem).*");
			PORTNAMES_REGEXP = Pattern.compile("cu.(serial|usbserial|usbmodem).*");
			PORTNAMES_PATH = "/dev/";
			break;
		}
	 ...
	 */
	private static String[] getUnixBasedPortNames(String searchPath, Pattern pattern,
			Comparator<String> comparator) {
		searchPath = (searchPath.equals("") ? searchPath : (searchPath.endsWith("/") ? searchPath
				: searchPath + "/"));
			...
			if (files.length > 0) {
				...
				for (File file : files) {
					...
					if (!file.isDirectory() && !file.isFile() && pattern.matcher(fileName).find()) {
						...
						long portHandle = serialInterface.openPort(portName, false);// Open
																					// port
																					// without
																					// TIOCEXCL
