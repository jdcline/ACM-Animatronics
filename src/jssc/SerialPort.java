...
public class SerialPort {
	...
	public boolean openPort() throws SerialPortException {
		...
		if (portName != null) {
			boolean useTIOCEXCL = (System
					.getProperty(SerialNativeInterface.PROPERTY_JSSC_NO_TIOCEXCL) == null && System
					.getProperty(SerialNativeInterface.PROPERTY_JSSC_NO_TIOCEXCL.toLowerCase()) == null);
			portHandle = serialInterface.openPort(portName, useTIOCEXCL);// since 2.3.0 -> (if JSSC_NO_TIOCEXCL defined,
																			// exclusive lock for serial port will be
																			// disabled)
		} else {
			throw new SerialPortException(portName, "openPort()",
		...
		if (portHandle == SerialNativeInterface.ERR_PORT_BUSY) {
			...
			throw new SerialPortException(portName, "openPort()",
					SerialPortException.TYPE_PERMISSION_DENIED);
	...
	public boolean setParams(int baudRate, int dataBits, int stopBits, int parity, boolean setRTS,
		...
		return serialInterface.setParams(portHandle, baudRate, dataBits, stopBits, parity, setRTS,
				setDTR, flags);
	}
	 ...
	 *
	 * @return If input buffer is empty <b>null</b> will be returned, else byte array with all data from port converted
	 *         to String
	 *
	 * @throws SerialPortException
	 *
	 ...
	 * value, if <b>overwriteMask == false</b> and mask has been already assigned the new <b>mask</b> value will be
	 * ignored,
			...
			while (!threadTerminated) {
				...
				for (int i = 0; i < eventArray.length; i++) {
					if (eventArray[i][0] > 0 && !threadTerminated) {
						 ...
						 * try {
						 * methodErrorOccurred.invoke(eventListener, new Object[]{new SerialPortException("port",
						 * "method", "exception")});
						 * }
						 * catch (Exception ex) {
			...
			while (!super.threadTerminated) {
				...
				for (int i = 0; i < eventArray.length; i++) {
					...
					if (eventType > 0 && !super.threadTerminated) {
						...
						if (sendEvent) {
							eventListener.serialEvent(new SerialPortEvent(portName, eventType,
									eventValue));
						}
					}
				}
